// Supabase Edge Function: Generate Quiz with AI
// Genera quizzes inteligentes basados en el material del usuario
...
serve(async (req) => {
  ...
  try {
    ...
    // Generar quiz
    const quizQuestions = await generateQuiz(context)
...
async function generateQuiz(context: string) {
  ...
  if (provider === 'demo') {
    return [
      {
        question: '[Modo Demo] ¿Cuál es el tema principal del material?',
        options: ['Tema A', 'Tema B', 'Tema C', 'Tema D'],
        correctIndex: 0,
        explanation: '[Modo Demo] Configura un proveedor de IA para generar preguntas reales.',
        difficulty: 'medium'
      }
    ]
  }

  const prompt = `Basándote en el siguiente material, genera 10 preguntas de opción múltiple.
...
Las preguntas deben ser claras, con opciones plausibles y explicaciones educativas.`
  ...
  if (provider === 'openai') {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Eres un experto en educación que crea preguntas efectivas.' },
